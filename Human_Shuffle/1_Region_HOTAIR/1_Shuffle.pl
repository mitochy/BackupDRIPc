...
my $dripcFileInput = "/data/mitochi/Work/Project/DRIPc/bed/drip_hotair.bed";
####################################################
# 1. Parse RNA-seq
...
my $genomicFile = "Genomic/genomic_$feature.bed"; die "Died undefined genomic $genomicFile for $dripcName\n" unless -e $genomicFile;
...
if ($input =~ /intergenic/) {
	...
	foreach my $dripName (keys %gene) {
		...
		for (my $i = 0; $i < 500; $i++) {
			...
			$strand2 = "-" if rand() < 0.5;
			print $out "$chr2\t$randStart\t$randEnd\t$name\t$value\t$strand2\tORIG=$name,TWIN=$name2,SHUFFLE=$i\n";
		}
		
		$count_Done ++;
	}
	#runbash("bedtools intersect -u -a $dripcName\_Shuffled.bed -b /data/mitochi/Work/Project/DRIPc/bed/NT2_DRIP_Peaks.bed | sort -k1,1 -k2,2n > $dripcName\_Shuffled_noDRIPc.bed");
}
else {
	my %data;
	my $last_index = "INIT";
	...
	foreach my $dripName (sort {$gene{$a}{rna} <=> $gene{$b}{rna}} keys %gene) {
		...
		#$count_notused_lowdripc ++ if $value < 5;
		...
		for (my $i = 0; $i < 500; $i++) {
			...
			print $out "$chr2\t$randStart\t$randEnd\t$name\t$value\t$strand2\tORIG=$gene,TWIN=$randGene,SHUFFLED=$i\n";
		}
	...
	#runbash("bedtools intersect -v -a $dripcName\_Shuffled.bed -b /data/mitochi/Work/Project/DRIPc/bed/NT2_DRIP_Peaks.bed | sort -k1,1 -k2,2n > $dripcName\_Shuffled_noDRIPc.bed");
}
close $out;
####################################################
...
#runbash("map_wig_to_bed.pl -w /data/mitochi/Work/Project/DRIPc/wig/NT2_hg19DRIP.wig -m -r /data/mitochi/Work/Cache/ $outputName.bed");
#runbash("mv NT2_hg19DRIP_$outputName.txt $dripcName.txt");
=comment
# c. Print out values 
$input = "$dripcName.txt";
my $randomTooBig    = 0;
...
foreach my $name (keys %data) {
	$lessThanTwoHundred ++ if $data{$name}{count} < 200;
	$mean += $data{$name}{count};
	$totalPeak ++;
	if ($data{$name}{count} >= 200) {
		for (my $i = 0; $i < 200; $i++) {
