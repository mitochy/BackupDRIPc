...
runbash("bedtools_bed_change.pl -a -x -20000 -y -1 -i merged.bed -o anti20k.bed");
# Intersect with gene
runbash("bedtools intersect -v -a anti20k.bed -b $bedFile > anti20k_nogene.bed");
# Change 3' to 10k distance
runbash("bedtools_bed_change.pl -x 10000 -i anti20k_nogene.bed -o anti10k_nogene.bed && rm anti20k_nogene.bed && rm anti20k.bed");
# intersect with DRIPc bed file that has REVERSE strand
runbash("bedtools intersect -S -wao -a anti10k_nogene.bed -b $dripcFile | grep -v -P \"\\t-1\\t\" > anti10k_nogene_dripc.bed");
# Calculate distance from TTS, size of peak, etc
my %data;
open (my $in2, "<", "anti10k_nogene_dripc.bed") or die "Cannot read from anti10k_nogene_dripc.bed: $!\n";
while (my $line = <$in2>) {
	chomp($line);
	...
	# Peak position
	my $startDist = $strand eq "+" ? $end2 - $end : $start - $start2;
	my $endDist   = $strand eq "+" ? $start2 - $end   : $start - $end2;
...
# Parse DRIPC data
my %dripc;
open (my $in1, "<", "anti10k_nogene_dripc.bed") or die "Cannot read from anti10k_nogene_dripc.bed: $!\n";
while (my $line = <$in1>) {
	chomp($line);
	my ($chr, $start, $end, $name, $zero, $strand) = split("\t", $line);
	$dripc{$name}{chr} = $chr;
