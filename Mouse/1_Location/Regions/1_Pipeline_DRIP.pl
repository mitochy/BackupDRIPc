...
use strict; use warnings; use mitochy; use Statistics::Basic qw(:all); use R_toolbox; use Getopt::Std;
use vars qw($opt_i $opt_b $opt_g);
getopts("i:b:g:");
use Term::ANSIColor qw(:constants);
$Term::ANSIColor::AUTORESET = 1;


die "Usage: $0 -i <RegionFile e.g. NT2_DRIP.bed> -b <Gene BedFile e.g. hg19_gencode.bed> -g <Genomic BED3 file e.g. hg19_genomic.bed>\n" unless defined($opt_i) and defined($opt_b) and defined($opt_g);
die "\nERROR: Region File (-i) $opt_i does not exist!\n\n" if not -e ($opt_i);
my ($regionsFile, $bedFile, $genomeFile)  = ($opt_i, $opt_b, $opt_g);
...
runbash("bedtools_bed_change.pl -b -x 1 -y 3000 -i $fileName\_genomic_promoter.bed -o $fileName\_genomic_promoter_ext.bed");
...
runbash("bedtools_bed_change.pl -b -x 1 -y 3000 -i $fileName\_genomic_terminal.bed -o $fileName\_genomic_terminal_ext.bed");
...
# Move 
mkdir "$fileName\_Region"   if not -d "$fileName\_Region";
mkdir "$fileName\_Genomic" if not -d "$fileName\_Genomic";
runbash("mv $fileName\_region_*.bed $fileName\_Region");
runbash("cat $fileName\_genomic*.bed | grep -v track > $fileName\_AllGenomic.bed");
runbash("mv $fileName\_genomic*.bed $fileName\_Genomic");
