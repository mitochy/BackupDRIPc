...
if ($input =~ /intergenic/) {
	...
	runbash("bedtools intersect -v -a $dripcName\_Shuffled.bed -b /data/mitochi/Work/Project/DRIPc/bed/drip_3T3barlow.bed | sort -k1,1 -k2,2n > $dripcName\_Shuffled_noDRIPc.bed");
}
else {
	my %data;
	my $last_index = "INIT";
