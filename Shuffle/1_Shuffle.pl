...
use vars qw($opt_i $opt_r $opt_z $opt_t $opt_g $opt_d $opt_w $opt_s);
getopts("i:r:z:t:g:d:w:s");

my ($regionInput, $RNAInput, $zeroInput, $twinInput, $genomeInput) = ($opt_i, $opt_r, $opt_z, $opt_t, $opt_g);
die "usage: $0 [-s: turn off strand] -r <RNA.rpkm> -z <RNA_RNAzero.id> -t <RNA_RNAtwin.id> -g <Region_Genomic.bed> -i <Region.bed>

!! If PROMOTER/TERMINAL use bed/hg19_gencode_promoter.bed or bed/hg19_gencode_terminal.bed for -g GENOMIC region

" unless defined($opt_i) and defined($opt_r) and defined($opt_z) and defined($opt_t) and defined($opt_g);
...
while (my $line = <$in3>) {
	...
	# If it's promoter/terminal then use gene which start/end is closest to dripc peak
		...
		foreach my $genename (@names) {
			...
			die "$regionInput: Died at $genename not defined start\n" if not defined($start2);
...
if ($regionInput =~ /intergenic/) {
	...
	#runbash("bedtools intersect -v -a $regionName\_Shuffled.bed -b $dripInput | sort -T /data/mitochi/Work/SortTMP/ -k1,1 -k2,2n > $regionName\_Shuffled_noDRIPc.bed");
}
	...
	foreach my $dripName (sort {$gene{$a}{rna} <=> $gene{$b}{rna}} keys %gene) {
		...
		$count_notused_lowdripc ++ and next if $value < 5 and $regionInput =~ /dripc/i;
		...
		for (my $i = 0; $i < 1000; $i++) {
			...
			chomp($genomic);
                        ...
                        my $randStart;
                        my $randEnd;
                        if ($regionInput !~ /antisense/) {
                                $randStart = $strand eq $strand2 ? $start2 + $startpos : $start2 + $endpos;
                                $randEnd   = $strand eq $strand2 ? $end2   - $endpos   : $end2 - $startpos;
                        }
                        else {
                                $randStart = $strand eq $strand2 ? $start2 + $endpos     : $start2 + $startpos;
                                $randEnd   = $strand eq $strand2 ? $end2   - $startpos   : $end2 - $endpos;
                        }
...
close $out;
runbash("bedtools intersect -v -a $regionName\_Shuffled.bed -b $regionInput | sort -T /data/mitochi/Work/SortTMP/ -k1,1 -k2,2n > $regionName\_TEMP && mv $regionName\_TEMP $regionName\_Shuffled.bed");

die "$regionInput DONE!

If this is a shuffled DRIP or DRIPc run bedtools intersect against DRIP or DRIPc peak!
	bedtools intersect -v -a peak -b drip.bed > peak_nodrip

Then get the signal
	map_wig_to_bed_BIG.pl -w DRIP -m $regionName\_Shuffled.bed\n";
...
PROMOTER/TERMINAl/ANTISENSE SCHEME:

Original:
---------------------T-----[---------]-------
Random TSS, same place:
---------------------T-----[---------]-------
