...
if ($input =~ /intergenic/) {
	...
	runbash("bedtools intersect -v -a $dripcName\_Shuffled.bed -b /data/mitochi/Work/Project/DRIPc/bed/dripc.bed | sort -k1,1 -k2,2n > $dripcName\_Shuffled_noDRIPc.bed");
}
else {
	my %data;
	my $last_index = "INIT";
	...
	runbash("bedtools intersect -v -s -a $dripcName\_Shuffled.bed -b /data/mitochi/Work/Project/DRIPc/bed/dripc.bed | sort -k1,1 -k2,2n > $dripcName\_Shuffled_noDRIPc.bed");
}
close $out;
####################################################
...
my $outputName = "$dripcName\_Shuffled_noDRIPc.bed";
runbash("grep + $outputName | sort -k1,1 -k2,2n > $outputName.pos");
runbash("grep - $outputName | sort -k1,1 -k2,2n > $outputName.neg");
# b. Run map_wig_to_bed
runbash("map_wig_to_bed.pl -w /data/mitochi/Work/Project/DRIPc/wig/NT2_DRIPc_pos.wig -m -r /data/mitochi/Work/Cache/ $outputName.pos");
runbash("map_wig_to_bed.pl -w /data/mitochi/Work/Project/DRIPc/wig/NT2_DRIPc_neg.wig -m -r /data/mitochi/Work/Cache/ $outputName.neg");
# c. Cat the result togetehr
runbash("cat NT2_DRIPc_pos_$dripcName\_Shuffled_noDRIPc.txt NT2_DRIPc_neg_$dripcName\_Shuffled_noDRIPc.txt > $dripcName.txt");
# d. Print out values 
$input = "$dripcName.txt";
my $randomTooBig    = 0;
...
foreach my $name (keys %data) {
	$lessThanThousand ++ if $data{$name}{count} < 1000;
