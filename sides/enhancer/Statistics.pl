...
my $shuffleInput = "/data/mitochi/Work/Project/DRIPc/sides/enhancer/MappedShuffled/$fileName.txt"; die "Shuffled $shuffleInput does not exist\n" if not -e $shuffleInput;
my ($histone, $feature) = $fileName =~ /^(\w+)_enh_(\w+)$/; die "Histone and/or Feature undefined\n" unless defined($histone) and defined($feature);
...
for (my $i = 0; $i < @random; $i++) {
	...
	push(@fold, ($medianOrig+1) / ($medianRandom+1));
	$nge++ if $medianOrig <= $medianRandom*1.15;
	$nle++ if $medianOrig*1.15 >= $medianRandom;
	$nrun++;
}
my $fold = join("\t", @fold);
...
print $outFold "$feature\t$histone\t$pvalUp $pvalDown\tHigher\t$medianFold\n" if $pvalUp <= 0.05;
print $outFold "$feature\t$histone\t$pvalUp $pvalDown\tLower\t$medianFold\n" if $pvalDown <= 0.05;
print $outFold "$feature\t$histone\t$pvalUp $pvalDown\tNone\t$medianFold\n" if $pvalUp > 0.05 and $pvalDown > 0.05;
close $outFold;
