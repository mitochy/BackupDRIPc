...
my ($feature)   = $dripcName =~ /enh_(\w+)$/; die "Undefined Feature from dripcname $dripcName (is it dripc_\\w+?)\n" unless defined($feature);
...
runbash("bedtools intersect -wb -a $input -b $genomicFile | cut -f1-5,13,15 | perl -pi -e \'s/^\(.+\)\\t\(.+\)\\t\(.+\)\\t\(.+\)\\t\(.+\)\\t\(.+\)\\t\(.+\)\$/\$1\\t\$2\\t\$3\\t\$4\\t\$5\\t\$7\\t\$6\/\' > $dripcName.name");
