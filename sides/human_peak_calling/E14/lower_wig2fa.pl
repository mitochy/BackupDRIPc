...
# CHANGE SYMBOL DEFINITION HERE
sub convert {
	my ($number) = @_;
	return("N") if $number == 0;
	return("L") if $number <= 5;
  return("O") if $number <= 12;
  return("M") if $number <= 20;
  return("H") if $number <= 35;
  return("S") if $number > 35;
  ...
  my ($name) = $input =~ /\/{0,1}(.+)$/ ; # to get filename
	...
	while (my $line = <$in>) {
		...
		if ($line =~ /Step/) {
			...
			($chr) = $line =~ /chrom(.+)/  if not defined($chr);
			print $out "\n" if $chr ne $lastchr and $chr ne "INIT";
			$span = 1 if not defined($span);
			$lastpos = 0 if $chr ne $lastchr;
