...
my %exon1 = %{parse_count("chr4_rep1_hg19Gencode_exon.rnacount")};
my %exon2 = %{parse_count("chr4_rep2_hg19Gencode_exon.rnacount")};
my %intron1 = %{parse_count("chr4_rep1_hg19Gencode_intron.rnacount")};
my %intron2 = %{parse_count("chr4_rep1_hg19Gencode_intron.rnacount")};

# Average RNAcount from both reps
...
foreach my $gene (keys %exon) {
	...
	$exon{$gene}{count} /= $total_basepair;
...
foreach my $gene (keys %intron) {
	...
	$intron{$gene}{count} /= $total_basepair;
}
%intron1 = ();
%intron2 = ();
...
good = df[which(df$Intron_count > 0.005 & df$Exon_count > 0.005 & df$Ratio >= 0.2),]
bad = df[which(!(df$Name %in% good$Name)),]
good$label="unspliced"
bad$label="spliced"
temp = rbind(good,bad)
png("NT2_unspliced_all.pdf")
