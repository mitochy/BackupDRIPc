...
print "This is original\n" if $input1 =~ /orig/i or $pwd =~ /orig/i;
print "This is shuffled\n" if $input1 =~ /shuf/i or $pwd =~ /shuf/i;
...
while (my $line = <$in1>) {
	...
	# Get center of TSS
	my ($orig, $TSSchr, $TSSstart, $TSSend) = $info =~ /ORIG=(ENS\w+\.\d+),(chr\w+),(\d+),(\d+)/ if $input1 =~ /orig/i or $pwd =~ /orig/i;
	my $shuf;
	   ($shuf, $TSSchr, $TSSstart, $TSSend) = $info =~ /TWIN=(ENS\w+\.\d+),(chr\w+),(\d+),(\d+)/ if $input1 =~ /shuf/i or $pwd =~ /shuf/i;
	...
	my $lineout = "$chr\t$start\t$end\t$orig\t$value\t$strand\t$info" if @arr == 7 and ($input1 =~ /orig/i or $pwd =~ /orig/i);
	$lineout = "$chr\t$start\t$end\t$value1\t$orig\t$value\t$strand\t$info" if @arr == 8 and ($input1 =~ /orig/i or $pwd =~ /orig/i);
	$lineout = "$chr\t$start\t$end\t$shuf\t$value\t$strand\t$info" if @arr == 7 and ($input1 =~ /shuf/i or $pwd =~ /shuf/i);
	$lineout = "$chr\t$start\t$end\t$value1\t$shuf\t$value\t$strand\t$info" if @arr == 8 and ($input1 =~ /shuf/i or $pwd =~ /shuf/i);
	...
	#print $out2 "$fileName1\t$origName\t$shufName\n";
}
close $in1;
close $out1;
close $out2;
