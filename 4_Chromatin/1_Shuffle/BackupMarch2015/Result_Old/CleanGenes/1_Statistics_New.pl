...
while (my $line = <$in2>) {
	chomp($line);
	next if $line =~ /#/;
	my ($chr, $start, $end, $value, $name, $dripcvalue) = split("\t", $line);
	my ($gene) = $line =~ /TWIN=(\w+\.\d+),chr/;
	my ($orig, $chr2, $start2, $end2) = $line =~ /ORIG=(\w+\.\d+),(chr\w+),(\d+),(\d+);/;
	my @arr = split("\t", $line);	
	$totalDRIPc ++;
	...
	if (not defined($data{$orig})) {
		$undefined ++;
		next;
	}
	else {
		#$data{$name}{orig} = $value;
		$data{$orig}{orig} = $value;
		$data{$orig}{dripc} = $dripcvalue;
		$data{$orig}{coor} = "$chr2\t$start2\t$end2";
	}
}
close $in2;
...
foreach my $name (keys %data) {
	...
	my $tag = $pvalUp <= 0.05 ? "BIGGER" : $pvalDown <= 0.05 ? "LOWER" : "";
	$temp{$name}{name} = "$tag\t$valueOrig\t$rna{$name}\t$valueDRIPc\t$name\tpUp $pvalUp\tpDown $pvalDown\tNGE $nge\tNLE $nle\t$data{$name}{coor}";
	$temp{$name}{val} = $valueDRIPc;
}
foreach my $name (sort {$temp{$b}{val} <=> $temp{$a}{val}} keys %temp) {
