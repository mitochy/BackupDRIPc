...
theme_blank\$axis.line = element_blank()
theme_blank\$axis.ticks.x = element_blank()
theme_blank\$axis.ticks.y = element_line(size=1)
theme_blank\$strip.text <- element_blank()
theme_blank\$axis.text.x <- element_blank()
...
xlab(\"\") + ylab(\"\") + coord_cartesian(ylim=c(ymin,ymax))# + theme_blank +
