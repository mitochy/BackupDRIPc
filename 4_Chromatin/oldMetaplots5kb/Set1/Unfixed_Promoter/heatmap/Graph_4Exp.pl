...
for (my $i = 0; $i < @files; $i++) {
	...
	#df = read.table(\"$file\",fill=TRUE); df = df[,-1];
	df[is.na(df)] = 0 
	length = dim(df)[2]
	...
	#df.mean = colMedian(df,na.rm=TRUE); #Median
	df.mean = colMeans(df,na.rm=TRUE);  #Mean
	df.pos=numeric(0);
	df.neg=numeric(0);
	for (i in 1:dim(df)[2]) {
		#a = apply(matrix(sample(df[,i], rep=TRUE, 10*length(df[,i])), nrow=20), 1, median) #Median
		a = apply(matrix(sample(df[,i], rep=TRUE, 10*length(df[,i])), nrow=20), 1, mean) #Mean
		df.pos[i]=as.numeric(quantile(a,0.95))
		df.neg[i]=as.numeric(quantile(a,0.05))
	}
	#x = seq(-4900,4900,by=100)
	#x = seq(-1900,1900,by=100)
