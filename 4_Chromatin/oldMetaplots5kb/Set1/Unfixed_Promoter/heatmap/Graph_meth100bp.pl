...
my $graphScript = defined($opt_g) ? $opt_g : "Graph_4Exp_meth100bp.pl";
...
while (my $line = <$in>) {
	...
	next if $line =~ /#/;
	my ($name) = split("\t", $line);
	my ($name2) = $name =~ /^(ENS\w+\.\d+)_/ if $name =~ /_/;
...
runbash("./Graph_4Exp_meth100bp.pl $fileNameOrig\_high.temp");
...
THIS SCRIPT DOES NOT CONVERT NA TO 0!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
