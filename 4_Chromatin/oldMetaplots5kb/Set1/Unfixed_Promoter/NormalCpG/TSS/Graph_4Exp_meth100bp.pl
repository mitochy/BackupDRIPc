...
# COLOR
my $color = color();
$ymax = $input =~ /dens/i ? 1.2 : $input =~ /cont/i ? 1   : $input =~ /skew/i ? 0.25 : $input =~ /meth/i ? 100 : "max(dat\$Val)";
$ymin = $input =~ /dens/i ? 0.2 : $input =~ /cont/i ? 0.2 : $input =~ /skew/i ? -0.1 : $input =~ /meth/i ? 0   : "min(dat\$Val)";
$Rscript .= "
$distR
