...
($histone, $tag, $feature, $driptype, $level) = $filename =~ /^(\w+)_(\w+)_(\w+)_(\w+)_(\w+)$/ if not defined($driptype);
...
for (my $i = 0; $i < @files; $i++) {
	...
	($histone, $tag, $feature, $driptype, $level) = $filename2 =~ /^(\w+)_(\w+)_(\w+)_(\w+)_(\w+)$/ if not defined($driptype);
	...
	df.pos  = apply(df,2,sd,na.rm=TRUE)#/sqrt(length(df.mean)))
	df.pos  = df.mean + (df.pos / e)
	df.neg  = apply(df,2,sd,na.rm=TRUE)#/sqrt(length(df.mean)))
...
theme_bw() + ggtitle(\"$histone at $feature\")# + coord_cartesian(xlim=c(-2000,2000),ylim=c(0,100))
dev.off()
";
