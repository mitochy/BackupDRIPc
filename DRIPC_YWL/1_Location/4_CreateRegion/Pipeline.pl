...
my $dripcFile  = "/data/mitochi/Work/Project/DRIPc/bed/dripc_neat1.bed";
...
# 10. Get genomic_intergenic.bed
print RED "10. Getting genomic_intergenic.bed\n";
runbash("cat genomic_promoter.bed genomic_promoter_ext.bed genomic_terminal.bed genomic_terminal_ext.bed genomic_both.bed genomic_both_ext.bed genomic_genebody.bed genomic_antisense.bed genomic_antisense_ext.bed> Fixed.bed");
...
runbash("bedtools subtract -s -a $dripcFile -b FixedDripc.bed > dripc_temp");
runbash("bedtools intersect -S -a dripc_temp -b $bedFile > dripc_antisense_other.bed");
...
sub get_feature {
	my ($input) = @_;
	return("PROMOTER") if $input =~ /dripc_promoter.bed/;
	return("PROMOTER_EXT") if $input =~ /dripc_promoter_ext.bed/;
	return("TERMINAL") if $input =~ /dripc_terminal.bed/;
	return("TERMINAL_EXT") if $input =~ /dripc_terminal_ext.bed/;
	return("BOTH") if $input =~ /dripc_both.bed/;
	return("BOTH_EXT") if $input =~ /dripc_both_ext.bed/;
	return("GENEBODY") if $input =~ /dripc_genebody.bed/;
	return("ANTISENSE_PROM") if $input =~ /dripc_antisense.bed/;
	return("ANTISENSE_GENIC") if $input =~ /dripc_antisense_other.bed/;
	return("INTERGENIC") if $input =~ /dripc_intergenic.bed/;
	return("ANTISENSE_EXT") if $input =~ /dripc_antisense_ext.bed/;
